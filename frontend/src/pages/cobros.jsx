...
export default function Cobros() {
  ...
  const ventasDisponiblesCliente = useMemo(() => {
    ...
    return ventasDisponibles.filter((venta) => String(venta.cliente) === String(clienteId));
  ...
  const ventasOptions = useMemo(
    () =>
      ventasDisponiblesCliente.map((venta) => ({
        ...
        label: `Venta #${venta.id} - ${formatDate(venta.fecha_venta)} - ${formatArs(venta.saldo_pendiente)}`,
      })),
  ...
  const hydrateFromVenta = (venta) => {
    ...
    setDetalles([{ venta: String(venta.id), montoAplicado: saldo }]);
  ...
  useEffect(() => {
    ...
    if (ventaData) hydrateFromVenta(ventaData);
  }, [ventaParam, ventas]);
  ...
  const handleAddDetalle = () => {
    setDetalles((prev) => [...prev, { venta: "", montoAplicado: "" }]);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const detallesPayload = detalles
      .filter((item) => item.venta || item.montoAplicado)
      .map((item) => ({ venta: Number(item.venta), monto_aplicado: item.montoAplicado }));

    const detallesInvalidos = detallesPayload.some(
      (item) => !item.venta || !item.monto_aplicado || Number(item.monto_aplicado) <= 0
    );
    if (detallesInvalidos) {
      setSubmitError("Completa venta y monto aplicado en cada imputacion.");
      return;
    }

    if (totalAplicado > totalMedios) {
      setSubmitError("La suma aplicada no puede superar el total del cobro.");
      return;
    }
        ...
        <form className="grid gap-3 xl:min-h-[calc(100vh-185px)] xl:grid-rows-[auto_minmax(0,1fr)_auto]" onSubmit={handleSubmit}>
          ...
          <section className={`${cardClass} grid gap-3 xl:grid-cols-[1fr_340px] xl:items-center`}>
            <div className="grid gap-2 md:grid-cols-[auto_auto_1fr] md:items-center">
              <div>
                <p className="text-xl font-semibold tracking-tight text-slate-800">Nuevo cobro</p>
              </div>
          ...
          <div className="grid min-h-0 gap-3 xl:grid-cols-[1.35fr_1fr]">
            <section className={`${cardClass} min-h-0`}>
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className={sectionTitleClass}>Ventas Aplicadas</p>
                  <h3 className="mt-0.5 text-xl font-semibold text-slate-800">Imputacion opcional</h3>
                </div>
                <div className="flex flex-col gap-2 sm:flex-row">
                  <Button type="button" className={secondaryButtonClass} onClick={handleAddDetalle} disabled={!clienteId}>
                    Agregar venta
                  </Button>
              ...
              <div className="mt-3 max-h-[260px] space-y-2 overflow-y-auto pr-1 xl:max-h-[300px]">
                {detalles.length === 0 ? (
                  <div className="rounded-[1rem] border border-dashed border-slate-300 bg-slate-50/70 px-4 py-6 text-center text-sm text-slate-500">
                    Sin ventas aplicadas por ahora.
                  </div>
                ) : null}
                {detalles.map((detalle, index) => (
                  <div key={`detalle-${index}`} className="rounded-[1rem] border border-slate-200 bg-white/85 p-2.5 shadow-[0_16px_40px_-34px_rgba(15,23,42,0.6)]">
                    <div className="mb-2 flex items-center justify-between gap-3">
                      <p className="text-sm font-semibold text-slate-700">Venta {index + 1}</p>
                    ...
                    <div className="grid gap-2 md:grid-cols-[1.6fr_0.72fr_0.72fr]">
                      <div>
                        <label className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Venta</label>
                        <div className="mt-1">
                          <SearchableSelect
                      ...
                      <div>
                        ...
                        <div className="mt-1 rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-2.5 text-sm font-medium text-slate-700">
                          {detalle.venta ? formatArs(ventasById[String(detalle.venta)]?.saldo_pendiente) : "-"}
                        </div>
                      </div>
                      <div>
          ...
          <section className={`${cardClass} grid gap-3 xl:grid-cols-[1.35fr_0.72fr]`}>
            <div>
              ...
              <textarea
                ...
                placeholder="Notas internas, referencia bancaria, observaciones del cobro..."
              />
            </div>
